...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (acao === "catalogo" || acao === "resumo") {
      const { data: planos, error: planosErro } = await admin.from("planos").select("id,nome,descricao,preco_referencia,periodo,duracao_dias,ordem,destaque,limites,plano_recursos(habilitado,limite,recurso:recursos(chave,nome,descricao))").eq("ativo", true).is("excluido_em", null).order("ordem").order("preco_referencia");
