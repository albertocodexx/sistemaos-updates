// supabase/functions/fiscal-documentos-provedor/index.ts
...
var pdfBase64Bytes = (valor) => {
  const bruto = texto(valor).replace(/^data:application\/pdf;base64,/i, "").replace(/\s/g, "");
  if (!bruto) return null;
  if (bruto.length > 28e6) throw new Error("DANFSe maior que o limite de 20 MB.");
  let binario = "";
  try {
    binario = atob(bruto);
  } catch (_) {
    throw new Error("DANFSe em Base64 invalido.");
  }
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: nota, error: notaErro } = await admin.from("notas_fiscais").select("id,empresa_id,status,numero,danfse_storage_path").eq("id", notaId).eq("empresa_id", empresaId).maybeSingle();
    ...
    const status = ["autorizada", "rejeitada", "cancelada", "processando"].includes(statusInformado) ? statusInformado : "processando";
    ...
    const ultimoErro = texto(dados.erro).slice(0, 1e3) || null;
    ...
    if (pdf) {
      ...
      const { error: uploadErro } = await admin.storage.from("documentos-fiscais").upload(caminhoPdf, pdf, { contentType: "application/pdf", cacheControl: "3600", upsert: true });
    ...
    const { data: atualizada, error: atualizarErro } = await admin.from("notas_fiscais").update(alteracoes).eq("id", notaId).eq("empresa_id", empresaId).select("id,status,numero,chave_acesso,danfse_storage_path,danfse_url,danfse_gerado_em,emitida_em").single();
