// supabase/functions/chamados-suporte/index.ts
...
var responder = (status, corpo) => new Response(JSON.stringify(corpo), { status, headers: cors });
var texto = (valor, limite = 8e3) => String(valor ?? "").trim().slice(0, limite);
var origensPublicas = /* @__PURE__ */ new Set(["login_pc", "login_celular"]);
var origensAutenticadas = /* @__PURE__ */ new Set(["config_pc", "config_celular"]);
var statusValidos = /* @__PURE__ */ new Set(["aberto", "em_atendimento", "resolvido", "fechado"]);
var prioridadesValidas = /* @__PURE__ */ new Set(["baixa", "normal", "alta", "critica"]);
var motivosValidos = /* @__PURE__ */ new Set([
  "trial_assinatura",
  "cobranca_pagamento",
  "acesso_login",
...
var preferenciasContatoValidas = /* @__PURE__ */ new Set(["whatsapp", "ligacao", "email"]);
var cargosEmpresaValidos = /* @__PURE__ */ new Set(["proprietario", "administrador", "gerente", "tecnico", "atendente", "financeiro", "outro"]);
async function contextoAutenticado(cliente) {
...
function podeAcessarAutenticado(chamado, autenticado) {
...
function camposChamado(dados) {
  ...
  const erro = !motivosValidos.has(motivo) ? "Selecione o motivo do chamado." : motivo === "outro" && motivoOutro.length < 3 ? "Descreva o outro motivo do chamado." : !/^\d{10,15}$/.test(telefone) ? "Informe um telefone ou WhatsApp com DDD." : !emailValido(email) ? "Confira o e-mail informado." : !preferenciasContatoValidas.has(preferenciaContato) ? "Selecione como prefere receber o retorno." : preferenciaContato === "email" && !email ? "Informe o e-mail escolhido para retorno." : !cargosEmpresaValidos.has(cargoEmpresa) ? "Selecione seu cargo na empresa." : cargoEmpresa === "outro" && cargoOutro.length < 2 ? "Informe seu cargo na empresa." : "";
  return {
    erro,
    motivo,
    motivoOutro: motivoOutro || null,
    telefone,
    email: email || null,
    preferenciaContato,
    cargoEmpresa,
    cargoOutro: cargoOutro || null,
    contato: [telefone, email].filter(Boolean).join(" \xB7 "),
    detalhes: { plataforma: plataforma || null, referencia: referencia || null, horario_contato: horarioContato || null }
  };
}
async function mensagensDoChamado(admin, chamadoId) {
  const { data, error } = await admin.from("chamado_mensagens").select("id,chamado_id,autor_tipo,autor_nome,mensagem,anexos,visualizado_cliente_em,visualizado_suporte_em,criado_em").eq("chamado_id", chamadoId).order("criado_em", { ascending: true }).limit(500);
  if (error) throw error;
  return data || [];
}
async function chamadoPorToken(admin, token) {
  if (!token) return null;
  const tokenHash = await sha256(token);
  const filtroToken = `token_hash.eq.${tokenHash},public_token.eq.${token}`;
  const { data, error } = await admin.from("chamados_suporte").select("*").or(filtroToken).is("excluido_em", null).maybeSingle();
  if (error) throw error;
  return data;
}
async function adicionarMensagem(admin, chamado, dados) {
  if (["resolvido", "fechado"].includes(chamado.status) && !dados.permitirEncerrado) {
    throw new Error("Este chamado j\xE1 foi encerrado e n\xE3o aceita novas mensagens.");
  }
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (acao === "criar_publico") {
      ...
      if (!/^[a-z0-9-]{3,40}$/.test(codigo) || !origensPublicas.has(origem) || !/^[a-z0-9._-]{3,30}$/.test(usuario) || nome.length < 2 || mensagem.length < 10 || campos.erro) {
        return responder(400, { erro: campos.erro || "Informe empresa, usu\xE1rio, nome e uma descri\xE7\xE3o com pelo menos 10 caracteres." });
      }
      const { data: empresa, error: empresaErro } = await admin.from("empresas").select("id").eq("codigo", codigo).eq("ativo", true).maybeSingle();
      if (empresaErro || !empresa) return responder(404, { erro: "N\xE3o foi poss\xEDvel confirmar a empresa e o usu\xE1rio informados." });
      const { data: identidade } = await admin.from("identidades_login").select("usuario_id").eq("empresa_id", empresa.id).ilike("usuario", usuario).eq("ativo", true).maybeSingle();
      const { data: perfil } = identidade?.usuario_id ? await admin.from("perfis").select("id").eq("id", identidade.usuario_id).eq("empresa_id", empresa.id).eq("ativo", true).maybeSingle() : { data: null };
      if (!identidade || !perfil) return responder(404, { erro: "N\xE3o foi poss\xEDvel confirmar a empresa e o usu\xE1rio informados." });
      ...
      const { data: chamado, error } = await admin.from("chamados_suporte").insert({
        ...
        contato_nome: nome,
        contato_usuario: usuario,
        contato: campos.contato,
        telefone_contato: campos.telefone,
        email_contato: campos.email,
        preferencia_contato: campos.preferenciaContato,
        cargo_empresa: campos.cargoEmpresa,
        cargo_outro: campos.cargoOutro,
        motivo: campos.motivo,
        motivo_outro: campos.motivoOutro,
      ...
      await adicionarMensagem(admin, { id: chamado.id, status: "aberto" }, {
        autorTipo: "cliente",
        autorNome: nome || usuario || "Cliente",
        mensagem
      });
      await admin.from("auditoria_comercial").insert({
        empresa_id: empresa.id,
        autor_id: null,
        acao: "chamado_publico_criado",
    ...
    if (acao === "criar_autenticado") {
      ...
      const prioridade = prioridadesValidas.has(texto(dados.prioridade, 20)) ? texto(dados.prioridade, 20) : "normal";
      const campos = camposChamado(dados);
      if (!autenticado || !atual?.empresa_id || atual?.administrador_global || !origensAutenticadas.has(origem) || mensagem.length < 10 || campos.erro) {
        return responder(400, { erro: campos.erro || "N\xE3o foi poss\xEDvel identificar a empresa ou a descri\xE7\xE3o \xE9 muito curta." });
      }
      const agora = (/* @__PURE__ */ new Date()).toISOString();
      const { data: chamado, error } = await admin.from("chamados_suporte").insert({
        empresa_id: atual.empresa_id,
        origem,
        aberto_por: autenticado.usuario.id,
        public_token: null,
        token_hash: null,
      ...
      await adicionarMensagem(admin, { id: chamado.id, status: "aberto" }, {
        autorTipo: "cliente",
        autorId: autenticado.usuario.id,
        autorNome: texto(atual.perfil_nome, 120) || texto(atual.usuario, 60) || "Cliente",
        mensagem
      });
      await admin.from("auditoria_comercial").insert({
        empresa_id: atual.empresa_id,
        autor_id: autenticado.usuario.id,
        acao: "chamado_autenticado_criado",
        entidade: "chamado_suporte",
        entidade_id: chamado.id,
    ...
    if (acao === "acompanhar_publico") {
      ...
      if (!chamado) return responder(404, { erro: "Chamado n\xE3o encontrado neste aparelho." });
    ...
    if (acao === "listar_meus") {
      ...
      let consulta = admin.from("chamados_suporte").select("id,assunto,origem,status,prioridade,motivo,motivo_outro,criado_em,atualizado_em,ultima_mensagem_em,primeira_resposta_em,encerrado_em,resolucao,avaliacao").eq("empresa_id", atual.empresa_id).is("excluido_em", null);
    ...
    if (acao === "listar") {
      if (!suporte) return responder(403, { erro: "A\xE7\xE3o restrita ao suporte do Sistema OS." });
      const { data: chamados, error } = await admin.from("chamados_suporte").select("id,empresa_id,aberto_por,origem,contato_nome,contato_usuario,contato,telefone_contato,email_contato,preferencia_contato,cargo_empresa,cargo_outro,motivo,motivo_outro,detalhes_contato,mensagem,assunto,prioridade,status,criado_em,atualizado_em,ultima_mensagem_em,primeira_resposta_em,encerrado_em,resolucao,atendido_por,avaliacao").is("excluido_em", null).order("ultima_mensagem_em", { ascending: false }).limit(200);
      if (error) throw error;
      const ids = [...new Set((chamados || []).map((item) => item.empresa_id).filter(Boolean))];
      const { data: empresas, error: empresasErro } = ids.length ? await admin.from("empresas").select("id,codigo,nome_fantasia").in("id", ids) : { data: [], error: null };
      if (empresasErro) throw empresasErro;
      const { data: naoLidas, error: naoLidasErro } = (chamados || []).length ? await admin.from("chamado_mensagens").select("chamado_id").in("chamado_id", (chamados || []).map((item) => item.id)).eq("autor_tipo", "cliente").is("visualizado_suporte_em", null) : { data: [], error: null };
      ...
      return responder(200, { chamados: (chamados || []).map((chamado) => ({
        ...chamado,
        protocolo: protocolo(chamado.id),
        nao_lidas_suporte: contagem.get(chamado.id) || 0,
    ...
    if (acao === "listar_mensagens") {
      ...
      if (error) throw error;
      if (!chamado || chamado.excluido_em) return responder(404, { erro: "Chamado n\xE3o encontrado." });
      const chamadoToken = token ? await chamadoPorToken(admin, token) : null;
      if (!suporte && !podeAcessarAutenticado(chamado, autenticado) && chamadoToken?.id !== chamado.id) return responder(403, { erro: "Voc\xEA n\xE3o pode acompanhar este chamado." });
      return responder(200, { chamado: { ...semSegredos(chamado), protocolo: protocolo(chamado.id) }, mensagens: await mensagensDoChamado(admin, chamado.id) });
    }
    if (acao === "responder_publico") {
      ...
      const mensagem = await adicionarMensagem(admin, chamado, {
        autorTipo: "cliente",
        autorNome: chamado.contato_nome || chamado.contato_usuario || "Cliente",
        mensagem: dados.mensagem
      });
      await admin.from("auditoria_comercial").insert({
        empresa_id: chamado.empresa_id,
        autor_id: null,
        acao: "chamado_resposta_publica",
        entidade: "chamado_suporte",
        entidade_id: chamado.id,
    ...
    if (acao === "responder_autenticado") {
      ...
      const { data: chamado, error } = await admin.from("chamados_suporte").select("*").eq("id", chamadoId).maybeSingle();
      if (error) throw error;
      if (!chamado || chamado.excluido_em || !podeAcessarAutenticado(chamado, autenticado)) return responder(403, { erro: "Voc\xEA n\xE3o pode responder este chamado." });
      const mensagem = await adicionarMensagem(admin, chamado, {
        autorTipo: "cliente",
        autorId: autenticado?.usuario.id,
        autorNome: autenticado?.contexto?.perfil_nome || autenticado?.contexto?.usuario || "Cliente",
        mensagem: dados.mensagem
      });
      await admin.from("auditoria_comercial").insert({
        empresa_id: chamado.empresa_id,
        autor_id: autenticado?.usuario.id,
        acao: "chamado_resposta_cliente",
        entidade: "chamado_suporte",
        entidade_id: chamado.id,
        metadados: { mensagemId: mensagem.id }
      });
    ...
    if (acao === "responder_suporte") {
      ...
      const mensagem = await adicionarMensagem(admin, chamado, {
        autorTipo: "suporte",
        autorId: autenticado?.usuario.id,
        autorNome: autenticado?.contexto?.perfil_nome || "Suporte Sistema OS",
        mensagem: dados.mensagem
      });
      await admin.from("auditoria_comercial").insert({
        empresa_id: chamado.empresa_id,
        autor_id: autenticado?.usuario.id,
        acao: "chamado_resposta_suporte",
        entidade: "chamado_suporte",
        entidade_id: chamado.id,
        metadados: { mensagemId: mensagem.id }
      });
    ...
    if (acao === "marcar_visualizado") {
      ...
      const { error: marcarErro } = await admin.from("chamado_mensagens").update({ [campo]: (/* @__PURE__ */ new Date()).toISOString() }).eq("chamado_id", chamado.id).eq("autor_tipo", autorOposto).is(campo, null);
    ...
    if (acao === "avaliar") {
      ...
      const chamadoId = uuidValido(dados.chamadoId);
      const chamadoToken = token ? await chamadoPorToken(admin, token) : null;
      const { data: chamado, error } = chamadoToken ? { data: chamadoToken, error: null } : await admin.from("chamados_suporte").select("id,empresa_id,aberto_por,excluido_em").eq("id", chamadoId).maybeSingle();
      if (error) throw error;
      if (!chamado || chamado.excluido_em || !chamadoToken && !podeAcessarAutenticado(chamado, autenticado) || !Number.isInteger(nota) || nota < 1 || nota > 5) {
        return responder(400, { erro: "N\xE3o foi poss\xEDvel registrar esta avalia\xE7\xE3o." });
      }
      const { error: avaliacaoErro } = await admin.from("chamados_suporte").update({
        avaliacao: nota,
        avaliacao_comentario: texto(dados.comentario, 1e3) || null,
    ...
    if (acao === "atualizar") {
      ...
      if (!chamadoId || !statusValidos.has(status)) return responder(400, { erro: "Status de chamado inv\xE1lido." });
      ...
      const campos = { status, atualizado_em: agora };
      ...
      if (status !== statusAnterior && status === "em_atendimento") {
        ...
        const conclusao = resolucao || (status === "resolvido" ? "O atendimento foi conclu\xEDdo pelo suporte." : "O atendimento foi encerrado pelo suporte.");
        await adicionarMensagem(admin, { ...chamado, status }, {
          autorTipo: "suporte",
          autorId: autenticado?.usuario.id,
          autorNome: nomeSuporte,
          mensagem: `${status === "resolvido" ? "Chamado resolvido." : "Chamado fechado."} ${conclusao} Este chat foi encerrado e n\xE3o aceita novas mensagens.`,
    ...
    if (acao === "excluir") {
      if (!suporte) return responder(403, { erro: "A\xE7\xE3o restrita ao suporte do Sistema OS." });
      const chamadoId = uuidValido(dados.chamadoId);
      if (!chamadoId) return responder(400, { erro: "Chamado inv\xE1lido." });
      const { data: chamado, error: chamadoErro } = await admin.from("chamados_suporte").select("id,empresa_id,assunto,status").eq("id", chamadoId).maybeSingle();
      if (chamadoErro) throw chamadoErro;
      if (!chamado) return responder(404, { erro: "Chamado n\xE3o encontrado." });
      const { count: mensagens } = await admin.from("chamado_mensagens").select("id", { count: "exact", head: true }).eq("chamado_id", chamadoId);
      ...
      const { error: excluirErro } = await admin.from("chamados_suporte").update({
        ...
        excluido_por: autenticado?.usuario.id,
        status: "fechado",
        atualizado_em: (/* @__PURE__ */ new Date()).toISOString()
      }).eq("id", chamadoId);
    ...
    return responder(400, { erro: "A\xE7\xE3o inv\xE1lida." });
  } catch (erro) {
    console.error("[chamados-suporte]", erro instanceof Error ? erro.message : String(erro));
    return responder(500, { erro: "N\xE3o foi poss\xEDvel concluir o chamado agora." });
  }
});
