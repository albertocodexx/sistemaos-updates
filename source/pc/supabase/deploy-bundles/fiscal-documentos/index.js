// supabase/functions/fiscal-documentos/index.ts
...
var digitos = (valor, limite = 20) => texto(valor).replace(/\D/g, "").slice(0, limite);
var cnpjNormalizado = (valor) => texto(valor).toUpperCase().replace(/[^0-9A-Z]/g, "");
var cnpjValido = (valor) => /^[0-9A-Z]{12}\d{2}$/.test(valor);
var decimalOpcional = (valor) => {
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (contextoErro || !contexto?.empresa_id || contexto.administrador_global || !contextoUsuarioAtivo(contexto) || !licencaPermiteOperacao(contexto)) {
    ...
    if (contexto.recursos_habilitados?.fiscal_habilitado !== true) {
      return resposta(403, { erro: "A emiss\xE3o de NFS-e ainda n\xE3o est\xE1 liberada para esta empresa." });
    }
    ...
    const podeEmitir = temPermissao(contexto, "financeiro", "criar") || temPermissao(contexto, "financeiro", "editar");
    const podeCancelar = temPermissao(contexto, "financeiro", "editar");
    if (acao === "resumo" || acao === "listar") {
      if (!podeLerFinanceiro) return resposta(403, { erro: "Seu usu\xE1rio n\xE3o pode consultar documentos fiscais." });
    ...
    if (acao === "salvar_configuracao") {
      ...
      const tipoPrestador = ["mei", "juridica", "fisica"].includes(tipoInformado) ? tipoInformado : texto(dados.tipoPessoa) === "fisica" ? "fisica" : "juridica";
      ...
      const regimesValidos = /* @__PURE__ */ new Set(["mei", "simples_nacional", "lucro_presumido", "lucro_real", "autonomo", "outro"]);
      ...
      const apuracoesValidas = /* @__PURE__ */ new Set(["padrao", "simples_nacional", "nfse"]);
      ...
      const retencoesValidas = /* @__PURE__ */ new Set(["nao", "tomador", "intermediario"]);
      ...
      const rotasValidas = /* @__PURE__ */ new Set(["nfse_nacional", "municipal", "provedor"]);
      ...
      const { data: configuracaoAtual, error: configuracaoAtualErro } = await admin.from("configuracoes_fiscais").select("provedor,ambiente,status,metadados,ultimo_erro").eq("empresa_id", empresaId).maybeSingle();
      if (configuracaoAtualErro) throw configuracaoAtualErro;
      const metadadosAtuais = configuracaoAtual?.metadados && typeof configuracaoAtual.metadados === "object" && !Array.isArray(configuracaoAtual.metadados) ? configuracaoAtual.metadados : {};
      const tipoAnterior = texto(metadadosAtuais.tipo_prestador) || (texto(metadadosAtuais.tipo_pessoa) === "fisica" ? "fisica" : "juridica");
      const documentoAnteriorBruto = metadadosAtuais.documento_prestador || metadadosAtuais.cpf || metadadosAtuais.cnpj;
      const documentoAnterior = tipoAnterior === "fisica" ? digitos(documentoAnteriorBruto, 20) : cnpjNormalizado(documentoAnteriorBruto);
      const rotaAnterior = texto(metadadosAtuais.rota_emissao) || "nfse_nacional";
      const identidadeMudou = Boolean(configuracaoAtual && (tipoAnterior !== tipoPrestador || documentoAnterior !== documentoPrestador || texto(metadadosAtuais.codigo_municipio) !== codigoMunicipio || rotaAnterior !== rotaEmissao || texto(configuracaoAtual.ambiente) !== ambiente));
      ...
      const completo = Boolean(documentoCompleto && codigoMunicipio.length === 7 && inscricaoCompleta && codigoServico && regimeTributario && autorizacaoCompleta);
      ...
      const ultimoErro = manterAtivacao ? null : completo ? "Dados fiscais salvos. Solicite a ativacao tecnica e o teste de homologacao." : tipoPrestador === "fisica" && !cadastroMunicipalConfirmado ? "Confirme o cadastro municipal da pessoa fisica e complete os dados da NFS-e." : "Complete os dados fiscais e solicite a ativacao da NFS-e.";
      ...
      return resposta(200, {
        configuracao,
        mensagem: manterAtivacao ? "Configuracao da NFS-e atualizada; a ativacao existente foi mantida." : completo ? "Dados da NFS-e salvos. Solicite a ativacao fiscal para conectar e homologar a emissao." : "Dados da NFS-e salvos. Complete os campos pendentes antes da ativacao."
      });
    }
    if (acao === "solicitar_emissao") {
      if (!podeEmitir) return resposta(403, { erro: "Seu usu\xE1rio n\xE3o pode emitir documentos fiscais." });
      ...
      if (!["os", "venda", "compra", "avulsa"].includes(origemTipo) || !origemId || !Number.isFinite(valor) || valor <= 0 || !descricao) {
        return resposta(400, { erro: "Informe documento, valor e descricao validos." });
      }
      const { data: existente, error: existenteErro } = await admin.from("notas_fiscais").select("id,origem_tipo,origem_id,valor,descricao,status,numero,chave_acesso,url_consulta,pdf_url,danfse_url,danfse_storage_path,danfse_gerado_em,created_at").eq("empresa_id", empresaId).eq("origem_tipo", origemTipo).eq("origem_id", origemId).maybeSingle();
      ...
      const { data: config, error: configErro } = await admin.from("configuracoes_fiscais").select("status,provedor").eq("empresa_id", empresaId).maybeSingle();
      ...
      const payload = {
        ...
        observacoes: texto(dados.observacoes).slice(0, 1e3)
      };
      const { data: nota, error } = await admin.from("notas_fiscais").upsert({
        ...
        descricao,
        status,
        provedor: config?.provedor || "nfse_nacional",
        payload,
        ultimo_erro: status === "aguardando_configuracao" ? "Configure os dados da NFS-e e conclua a ativacao fiscal." : null,
        updated_at: (/* @__PURE__ */ new Date()).toISOString()
      }, { onConflict: "empresa_id,origem_tipo,origem_id" }).select("id,origem_tipo,origem_id,valor,descricao,status,numero,url_consulta,pdf_url,created_at").single();
      if (error) throw error;
      return resposta(200, { nota, mensagem: status === "na_fila" ? "NFS-e adicionada a fila de emissao." : "Solicitacao de NFS-e salva. Conclua a ativacao fiscal para autoriza-la." });
    }
    if (acao === "obter_danfse") {
      if (!podeLerFinanceiro) return resposta(403, { erro: "Seu usu\xE1rio n\xE3o pode consultar documentos fiscais." });
      const id = texto(dados.id);
      if (!id) return resposta(400, { erro: "Informe a NFS-e." });
      const { data: nota, error } = await admin.from("notas_fiscais").select("id,status,numero,chave_acesso,pdf_url,danfse_url,danfse_storage_path,danfse_gerado_em").eq("id", id).eq("empresa_id", empresaId).maybeSingle();
      if (error) throw error;
      ...
      if (nota.danfse_storage_path) {
        const { data: assinatura, error: assinaturaErro } = await admin.storage.from("documentos-fiscais").createSignedUrl(nota.danfse_storage_path, 300, { download: `DANFSe-${nota.numero || nota.id}.pdf` });
        if (assinaturaErro) throw assinaturaErro;
        url2 = urlHttps(assinatura?.signedUrl);
      }
      if (!url2) url2 = urlHttps(nota.danfse_url || nota.pdf_url);
      if (!url2) {
    ...
    if (acao === "cancelar_solicitacao") {
      if (!podeCancelar) return resposta(403, { erro: "Seu usu\xE1rio n\xE3o pode cancelar documentos fiscais." });
      const id = texto(dados.id);
      const { data: nota, error } = await admin.from("notas_fiscais").update({
        status: "cancelada",
        updated_at: (/* @__PURE__ */ new Date()).toISOString()
      }).eq("id", id).eq("empresa_id", empresaId).in("status", ["rascunho", "aguardando_configuracao", "na_fila"]).select("id,status").maybeSingle();
      if (error) throw error;
