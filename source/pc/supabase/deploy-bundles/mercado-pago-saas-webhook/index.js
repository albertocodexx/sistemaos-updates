// supabase/functions/mercado-pago-saas-webhook/index.ts
...
var bytesHex = (buffer) => Array.from(new Uint8Array(buffer)).map((valor) => valor.toString(16).padStart(2, "0")).join("");
var igualdadeConstante = (a, b) => {
  if (a.length !== b.length) return false;
  let diferenca = 0;
  for (let indice = 0; indice < a.length; indice += 1) diferenca |= a.charCodeAt(indice) ^ b.charCodeAt(indice);
  return diferenca === 0;
};
async function validarAssinatura(req, dataId, segredo) {
  ...
  const timestampMs = timestampRecebido < 1e12 ? timestampRecebido * 1e3 : timestampRecebido;
  if (Math.abs(Date.now() - timestampMs) > 10 * 60 * 1e3) return false;
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (topico === "merchant_order") {
      ...
      if (!ordemResposta.ok) return resposta(502, { erro: "Ordem ainda n\xE3o pode ser consultada." });
      const ordem = await ordemResposta.json().catch(() => ({}));
      paymentId = texto((ordem.payments || []).find((item) => item.status === "approved")?.id || ordem.payments?.[0]?.id);
    ...
    const { data: cobranca, error: cobrancaErro } = await admin.from("cobrancas_assinatura").select("id,empresa_id,plano_id,valor,moeda,status,aplicado_em,pagamento_provedor_id").eq("referencia_externa", referencia).maybeSingle();
    if (cobrancaErro) throw cobrancaErro;
    if (!cobranca) return resposta(200, { recebido: true, referencia_desconhecida: true });
    if (cobranca.aplicado_em && (texto(cobranca.pagamento_provedor_id) !== texto(pagamento.id) || !["approved", "refunded", "charged_back"].includes(pagamento.status))) {
      return resposta(200, { recebido: true, ignorado: true });
    }
    const valorRecebido = Number(pagamento.transaction_amount || 0);
    const moeda = texto(pagamento.currency_id);
    if (!Number.isFinite(valorRecebido) || moeda !== cobranca.moeda || Math.abs(valorRecebido - Number(cobranca.valor)) > 9e-3 || pagamento.status === "approved" && Number(pagamento.transaction_amount_refunded || 0) > 0) {
    ...
    const pagoEm = pagamento.date_approved || pagamento.date_created || (/* @__PURE__ */ new Date()).toISOString();
    const dadosMinimos = {
    ...
    atualizacao = cobranca.aplicado_em ? atualizacao.eq("pagamento_provedor_id", texto(pagamento.id)) : atualizacao.is("aplicado_em", null);
    ...
    if (status === "estornada" && cobranca.aplicado_em) {
      await admin.from("pagamentos_assinatura").update({ status: "estornado", updated_at: (/* @__PURE__ */ new Date()).toISOString() }).eq("cobranca_id", cobranca.id);
